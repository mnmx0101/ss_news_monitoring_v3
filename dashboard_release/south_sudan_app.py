"""
South Sudan GDELT Risk Monitoring Dashboard - Unified
"""
import sys
...
METRIC_LABELS = {k: v.get("title", k) for k, v in METRIC_CONFIG.items() if isinstance(v, dict)}
...
def load_panel():
    try:
        return pd.read_csv(RESULTS_DIR / "panel_df.csv")
    except Exception:
        return pd.DataFrame()
...
panel_df_raw = load_panel()
conv_df_raw  = load_convergence()
summ_df_raw  = load_summaries()
peak_df_raw  = load_peaks()

if panel_df_raw.empty:
    st.error("No panel data found. Run `python run_pipeline.py` first.")
    st.stop()

# ── Sidebar Global Filters ───────────────────────────────────────────────────
st.sidebar.header("Global Filters")
st.sidebar.markdown("Use these filters to dynamically slice data across all dashboard tabs.")

all_months = sorted(panel_df_raw['year_month'].dropna().unique())
if len(all_months) >= 2:
    ...
    start_date = end_date = all_months[0] if all_months else "2020-01"

all_adm1s = sorted(panel_df_raw['ADM1_EN'].dropna().unique())
selected_adm1s = st.sidebar.multiselect("Select ADM1 Regions:", all_adm1s, default=all_adm1s)

extra = ["n_protest", "n_fight", "n_mass_violence", "goldstein_mean"]
all_metrics = [m for m in ORDERED_METRICS if m in panel_df_raw.columns] + \
              [m for m in extra if m in panel_df_raw.columns and m not in ORDERED_METRICS]

selected_metric = st.sidebar.selectbox(
    "Primary Metric (Tabs 1-4):",
    all_metrics,
...
def filter_global(df, date_col='year_month', adm1_col='ADM1_EN'):
    if df.empty: return df
    mask = pd.Series(True, index=df.index)
    if date_col in df.columns:
        mask &= (df[date_col] >= start_date) & (df[date_col] <= end_date)
    if adm1_col in df.columns:
        # Keep if matches selected, or if missing (national aggs)
        mask &= df[adm1_col].isin(selected_adm1s) | df[adm1_col].isna()
    return df[mask]

# Apply Filters
panel_df = filter_global(panel_df_raw)
conv_df  = filter_global(conv_df_raw)
summ_df  = filter_global(summ_df_raw, adm1_col='adm1')
peak_df  = filter_global(peak_df_raw)
...
with tab0:
    ...
    st.markdown("Overview of filtered data per source (respecting global sidebar filters).")
    ...
    for src in FIVE_SOURCES:
        ...
        rows.append({
            ...
            "Total events": int(p["n_events_total"].sum()) if "n_events_total" in p else 0,
    ...
    geo_rows = []
    for src in FIVE_SOURCES:
        if src not in panels: continue
        p = panels[src]
        if "geo_match" not in p.columns: continue
...
with tab1:
    ...
    st.markdown(f"**Metric Displayed:** {METRIC_LABELS.get(selected_metric, selected_metric)}")

    agg_func_t1 = "mean" if selected_metric in ("avg_tone_mean", "goldstein_mean") else "sum"

    trend_parts = []
    for src in FIVE_SOURCES:
        if src not in panels or panels[src].empty: continue
        p = panels[src]
        if selected_metric not in p.columns: continue
        agg = p.groupby(["year_month", "ADM1_EN"])[selected_metric].agg(agg_func_t1).reset_index()
        agg["source"] = src
    ...
    if trend_parts:
        ...
        if not trend_df.empty:
            x_tick_vals_t1 = trend_df["year_month"].sort_values().unique()[::max(1, len(trend_df['year_month'].unique())//6)].tolist()
            chart = (
                alt.Chart(trend_df)
                .mark_bar()
                .encode(
                    x=alt.X("year_month:O", title="Month", axis=alt.Axis(labelAngle=-90, labelFontSize=5, values=x_tick_vals_t1)),
                    y=alt.Y(f"{selected_metric}:Q", title=METRIC_LABELS.get(selected_metric, selected_metric)),
                    color=alt.Color("ADM1_EN:N", title="ADM1"),
                    tooltip=["year_month", "ADM1_EN", f"{selected_metric}:Q"],
                )
                .properties(width=220, height=300)
                .facet(
                    column=alt.Column("source:N", title="Source", sort=FIVE_SOURCES, header=alt.Header(labelFontWeight="bold")),
                )
...
with tab2:
    ...
    with col_mode:
        threshold_mode = st.radio("Threshold:", ["Static (full series)", "Dynamic (rolling)"], horizontal=True, key="t2_mode")
    with col_win:
        roll_win = st.select_slider("Rolling window (months):", [3, 6, 12], value=6, disabled=(threshold_mode != "Dynamic (rolling)"), key="t2_win")

    cfg = METRIC_CONFIG.get(selected_metric, {"agg":"sum","direction":"high"})

    with st.spinner("Computing alert matrices..."):
        if threshold_mode == "Static (full series)":
            matrices = get_alert_matrices(panels, selected_metric, cfg["agg"], cfg["direction"])
        else:
            matrices = get_alert_matrices_dynamic(panels, selected_metric, cfg["agg"], cfg["direction"], roll_win)
    ...
    if long_parts:
        ...
        alert_color = alt.Scale(domain=[0,1,2,3], range=[ALERT_COLORS[0], ALERT_COLORS[1], ALERT_COLORS[2], ALERT_COLORS[3]])
        all_months_sorted = sorted(alert_long["year_month"].unique())
        x_tick_vals = all_months_sorted[::max(1, len(all_months_sorted)//6)]

        heatmap = (
            alt.Chart(alert_long)
            .mark_rect()
            .encode(
                x=alt.X("year_month:O", title="Month", axis=alt.Axis(labelAngle=-90, labelFontSize=5, values=x_tick_vals)),
                y=alt.Y("ADM1_EN:N", title="ADM1"),
                color=alt.Color("alert_level:O", scale=alert_color, legend=alt.Legend(title="Level", labelExpr="datum.value == 0 ? 'normal' : datum.value == 1 ? 'alert' : datum.value == 2 ? 'alarm' : 'no data'")),
                tooltip=["year_month", "ADM1_EN", "alert_level:O", "source:N"],
            )
            .properties(width=200, height=350)
            .facet(column=alt.Column("source:N", title="Source", sort=FIVE_SOURCES, header=alt.Header(labelFontWeight="bold")))
        )
        st.altair_chart(heatmap)
    else:
        st.info("No heatmap data to render.")
...
with tab3:
    ...
    col_mode3, col_win3 = st.columns(2)
    with col_mode3:
        threshold_mode3 = st.radio("Threshold:", ["Static (full series)", "Dynamic (rolling)"], horizontal=True, key="t3_mode")
    with col_win3:
        roll_win3 = st.select_slider("Rolling window (months):", [3, 6, 12], value=6, disabled=(threshold_mode3 != "Dynamic (rolling)"), key="t3_win")

    cfg3 = METRIC_CONFIG.get(selected_metric, {"agg":"sum","direction":"high"})

    with st.spinner("Computing convergence..."):
        if threshold_mode3 == "Static (full series)":
            mat3 = get_alert_matrices(panels, selected_metric, cfg3["agg"], cfg3["direction"])
        else:
            mat3 = get_alert_matrices_dynamic(panels, selected_metric, cfg3["agg"], cfg3["direction"], roll_win3)
        if mat3:
            conv_matrix = compute_convergence(mat3)
            conv_matrix_named = conv_matrix.copy()
            conv_matrix_named.index.name = "year_month"
            conv_long = conv_matrix_named.reset_index().melt(id_vars=["year_month"], var_name="ADM1_EN", value_name="convergence_score")
            conv_long["convergence_score"] = conv_long["convergence_score"].astype(int)

            conv_color = alt.Scale(domain=list(range(len(CONV_COLORS))), range=CONV_COLORS)
            all_months_conv = sorted(conv_long["year_month"].unique())
            x_ticks_conv = all_months_conv[::max(1, len(all_months_conv)//6)]

            conv_heatmap = (
                alt.Chart(conv_long)
                .mark_rect()
                .encode(
                    x=alt.X("year_month:O", title="Month", axis=alt.Axis(labelAngle=-90, labelFontSize=6, values=x_ticks_conv)),
                    y=alt.Y("ADM1_EN:N", title="ADM1"),
                    color=alt.Color("convergence_score:O", scale=conv_color, title="# sources at alert+"),
...
with tab4:
    ...
    col_mode4, col_win4, col_roll4 = st.columns(3)
    with col_mode4:
        threshold_mode4 = st.radio("Threshold:", ["Static", "Dynamic"], horizontal=True, key="t4_mode")
    with col_win4:
        roll_win4 = st.select_slider("Alert window:", [3, 6, 12], value=6, disabled=(threshold_mode4 != "Dynamic"), key="t4_win")
    with col_roll4:
        trend_win = st.select_slider("Trend smoothing:", [3, 6, 12], value=6, key="t4_trend")

    cfg4 = METRIC_CONFIG.get(selected_metric, {"agg":"sum","direction":"high"})

    with st.spinner("Computing national incidence..."):
        if threshold_mode4 == "Static":
            mat4 = get_alert_matrices(panels, selected_metric, cfg4["agg"], cfg4["direction"])
        else:
            mat4 = get_alert_matrices_dynamic(panels, selected_metric, cfg4["agg"], cfg4["direction"], roll_win4)
        ...
        if mat4:
            ...
            if src_inc_parts:
                ...
                x_tick_vals_t4 = sorted(src_inc["year_month"].unique())[::max(1, len(src_inc["year_month"].unique())//6)]
                inc_chart = (
                    alt.Chart(src_inc)
                    .mark_bar(opacity=0.7)
                    .encode(
                        x=alt.X("year_month:O", title="Month", axis=alt.Axis(labelAngle=-90, labelFontSize=5, values=x_tick_vals_t4)),
                        y=alt.Y("n_adm1_alert:Q", title="# ADM1s at alert+"),
                        color=alt.Color("source:N", title="Source"),
                        tooltip=["year_month", "source", "n_adm1_alert"],
                    )
                    .properties(width=200, height=250)
                    .facet(column=alt.Column("source:N", sort=FIVE_SOURCES, header=alt.Header(labelFontWeight="bold")))
                )
            ...
            conv_long4 = conv4_named.reset_index().melt(id_vars=["year_month"], var_name="ADM1_EN", value_name="score")
            conv_long4["score"] = conv_long4["score"].astype(int)

            stacked_parts = []
            for level in range(1, 6):
                grp = conv_long4[conv_long4["score"] == level].groupby("year_month").size().reset_index(name="n_adm1")
                grp["conv_level"] = f"{level} src"
                stacked_parts.append(grp)
            stacked = pd.concat(stacked_parts, ignore_index=True)

            total_per_month = conv_long4[conv_long4["score"] >= 1].groupby("year_month").size().reset_index(name="total").sort_values("year_month")
            total_per_month["rolling"] = total_per_month["total"].rolling(trend_win, center=True, min_periods=1).mean()

            all_months4 = sorted(stacked["year_month"].unique())
            x_ticks4 = all_months4[::max(1, len(all_months4)//6)]

            bar4 = (
                alt.Chart(stacked)
                .mark_bar()
                .encode(
                    x=alt.X("year_month:O", title="Month", axis=alt.Axis(labelAngle=-90, labelFontSize=6, values=x_ticks4)),
                    y=alt.Y("n_adm1:Q", title="# ADM1s"),
                    color=alt.Color("conv_level:N", scale=alt.Scale(domain=[f"{n} src" for n in range(1, 6)], range=CONV_COLORS[1:6]), title="Convergence level"),
...
with tab5:
    ...
    if current_peaks.empty and not peak_df.empty:
        current_peaks = peak_df.rename(columns={"convergence_score": "score", "score": "convergence_score"})

    if current_peaks.empty:
        st.warning("No peaks identified. Please visit the **National Incidence** tab first.")
    else:
        ...
        with col_g2:
            ...
            gen_ym = st.selectbox("Period:", relevant_months, index=len(relevant_months)-1 if relevant_months else 0, key="gen_ym")
        with col_g3:
        ...
        def get_raw_articles_lazy(adm1, ym, source):
            raw_path = PROJECT_ROOT / "data" / "gdelt_raw.csv.gz"
            if not raw_path.exists(): return pd.DataFrame()
            from src.sources import categorize_source
            raw_iter = pd.read_csv(raw_path, compression='gzip', usecols=['ActionGeo_ADM2Code', 'SOURCEURL', 'MonthYear'], chunksize=50000)
            adm_lookup = panel_df_raw[['ActionGeo_ADM2Code', 'ADM1_EN']].drop_duplicates()
            ...
            for chunk in raw_iter:
                ...
                if not chunk.empty: filtered_parts.append(chunk)
            ...
            if st.button("Check Stats & Estimate Cost", type="secondary", use_container_width=True):
                with st.spinner("Retrieving article stats..."):
                    ...
                    if meta.empty:
                        ...
                        st.success("Article stats retrieved.")
    ...
    st.info("💡 **Note:** Displaying briefings filtered by global sidebar settings.")

    if summ_df.empty:
        st.write("No saved briefings match current filters.")
    else:
        st.caption(f"Showing {len(summ_df)} sample briefings")
        for _, row in summ_df.iterrows():
            with st.container(border=True):
                ...
                sev_color = {"low": "green", "medium": "orange", "high": "red", "critical": "red"}.get(str(severity).lower(), "gray")
