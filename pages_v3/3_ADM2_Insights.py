"""
ADM2 Insights V3 — County-level drilldown line chart with threshold bands and IPC event overlays.
"""
...
from ipc_events import expand_validation_events

st.title("📍 ADM2 — County-Level Insights")
st.markdown(
    "Drill into a specific county's article trend over time. "
    "Threshold bands and IPC crisis event overlays help validate whether media signals align with ground truth."
)
...
# Build ADM1 → ADM2 mapping for grouped selector
adm_lookup = (
    df_raw[["adm1_name_final", "adm2_name_final"]]
    .drop_duplicates()
    .sort_values(["adm1_name_final", "adm2_name_final"])
)
all_adm1 = sorted(adm_lookup["adm1_name_final"].unique())

# ── SIDEBAR CONTROLS ──────────────────────────────────────────────────────────
with st.sidebar:
    st.header("📊 Controls")

    st.markdown("#### 📍 State → County")
    st.caption("First select a state, then choose the county within it.")
    selected_adm1 = st.selectbox("State (ADM1)", options=all_adm1, label_visibility="collapsed")
    counties_in_state = sorted(adm_lookup[adm_lookup["adm1_name_final"] == selected_adm1]["adm2_name_final"].unique())
    selected_region = st.selectbox("County (ADM2)", options=counties_in_state, label_visibility="collapsed")

    st.markdown("#### 📋 Topic")
    st.caption("The thematic category to analyse.")
    ...
    if metric == "Sentiment Score":
        ...
        st.caption("Tone is required for Sentiment Score ('All' is disabled).")
    else:
        tone_opts = ["All", "Positive", "Negative", "Neutral"]
        st.caption("Filter articles by tone. 'All' includes every article.")
    ...
    N = st.slider("Min History (N months)", 1, 36, 12)
    st.caption("Counties with fewer than N months use nationally-scaled thresholds (⚠️).")
...
df_clean = df_filtered.drop_duplicates(subset=["adm2_name_final", "yearmon", "Label", "title"])
panel = df_clean.groupby(["adm2_name_final", "yearmon", "Label"]).agg(
...
if topic_panel.empty:
    st.warning(f"No data for **{selected_topic}** / tone **{tone}**.")
    st.stop()

# ── CLASSIFY ──────────────────────────────────────────────────────────────────
region_df = topic_panel[topic_panel["adm2_name_final"] == selected_region].copy()
if region_df.empty:
...
status_icon = {"Normal": "🟢", "Alert": "🟠", "Alarm": "🔴"}.get(latest_status, "⚪")
