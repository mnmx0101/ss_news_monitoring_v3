...
def classify_panel(topic_df, adm_col):
    """Apply region-level threshold classification with fallback."""
    if topic_df.empty:
        return topic_df
    ...
    result = (
        ...
        .apply(lambda g: classify_region(g, metric_col, method, method_params, c_p1, c_p2, c_med, N))
...
def build_heatmap(classified, display_col, label_title, height):
    ...
    return alt.Chart(hp).mark_rect().encode(
        ...
        color=alt.Color("status:N", scale=alt.Scale(
            domain=["No Data", "Normal", "Alert", "Alarm"],
...
# ── NATIONAL TS BUILDER ───────────────────────────────────────────────────────
def build_nat_ts(classified):
    nat = classified.groupby(["yearmon_date", "status"]).size().reset_index(name="n_regions")
    nat = nat[nat["status"].isin(["Alert", "Alarm"])]
    if nat.empty:
        return None
    return alt.Chart(nat).mark_bar().encode(
        x=alt.X("yearmon_date:T", title="Month"),
        y=alt.Y("n_regions:Q", title="Regions Affected"),
        color=alt.Color("status:N", scale=alt.Scale(domain=["Alert", "Alarm"], range=["#ff9800", "#f44336"])),
...
with tab_adm1:
    if df_filtered.empty:
        ...
        if topic_adm1.empty:
            ...
            nat_chart = build_nat_ts(classified_adm1)
            if nat_chart:
    ...
    if df_filtered.empty:
        ...
        if topic_adm2.empty:
            ...
            def classify_adm2(group):
                return classify_region(group, metric_col, method, method_params, c_p1, c_p2, c_med, N)
            ...
            hm_adm2 = alt.Chart(hp_adm2).mark_rect().encode(
                ...
                height=max(600, len(ordered_display) * 16),
                width="container"
            )
